...
function mettreslug($str){
  return mb_strtolower(preg_replace(array('/[^a-zA-Z0-9 \'-]/', '/[ -\']+/', '/^-|-$/','[-----|----|---|--]'),
  array('', '-', '','-'), remove_accent($str)));
}

function verifierInput($donner){
    $valide = trim(stripslashes(htmlspecialchars($donner)));
    return $valide;
}


// function mettreslug($urlString) {
//     $search = array('Ș', 'Ț', 'ş', 'ţ', 'Ş', 'Ţ', 'ș', 'ț', 'î', 'â', 'ă', 'Î', ' ', 'Ă', 'ë', 'Ë','é');
//     $replace = array('s', 't', 's', 't', 's', 't', 's', 't', 'i', 'a', 'a', 'i', '-', 'a', 'e', 'E','e');
...
//         try{
//          $resultat = $requete->execute( array($_POST['titre'], $_POST['description'], $_POST['contenu'], $_POST['image'],date("Y-m-d H:i:s"), mettreslug($_POST['titre'])));
//          if($resultat){
//              echo "<div style='color: green;'>Vous venez  de publier une nouvelle actualité.</div>";
//          	}
//          }
//               catch(Exception $e){
